...
def abrirSubmenuReportes(pVentana,pDonadores,pTiposSangre,pLugaresDonacion):
    ...
    crearBoton(ventanaReportes,"1.Donantes por provincia",lambda:reporteDonantesProvincia(pDonadores))
    crearBoton(ventanaReportes,"2.Por rango de edad",lambda:reporteRangoEdad(pDonadores))
    crearBoton(ventanaReportes,"3.Por tipo de sangre de una provincia",lambda:reporteTipoSangreProvincia(pDonadores,pTiposSangre))
    ...
    crearBoton(ventanaReportes,"6.¿A quién puede donar?",lambda:reportePuedeDonar(pDonadores,pTiposSangre))
    crearBoton(ventanaReportes,"7.¿De quién puede recibir?",lambda:reportePuedeRecibir(pDonadores,pTiposSangre))
    crearBoton(ventanaReportes,"8.Donantes NO activos",lambda:reporteDonantesNoActivos(pDonadores,pTiposSangre))
    crearBoton(ventanaReportes,"9.Lugares de donación",lambda:reporteLugaresDonacion(pDonadores,pLugaresDonacion))
...
def menuPrincipal(pDonadores):
    ...
    lugaresDonacion=cargarLugaresDonacionAux()
    ventana=Tk()
    ventana.title("Banco de Sangre")
    ventana.geometry("500x500")
    Label(ventana,text="BANCO DE SANGRE",font=("Century Gothic",14,"bold")).pack(pady=15)
    crearBoton(ventana,"1.Insertar donador",lambda:abrirInsertarDonador(ventana,pDonadores,tiposSangre,boton3,boton4,boton6))
    ...
    boton3=crearBoton(ventana,"3.Actualizar datos del donador",lambda:abrirActualizarDonador(ventana,pDonadores,tiposSangre))
    boton4=crearBoton(ventana,"4.Eliminar donador",lambda:ventanaEliminarDonador(pDonadores))
    crearBoton(ventana,"5.Insertar lugar de donación",lambda:abrirInsertarLugarDonacion(ventana,lugaresDonacion))
